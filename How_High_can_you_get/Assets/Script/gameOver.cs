...
public class gameOver : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		if (GUI.Button (new Rect (Screen.width / 2 + 430f, Screen.height / 3 + 0f, 100f, 50f), "Play Again")) {
			//26. load the main scene (which is 1 in the build settings)
			Application.LoadLevel (1);
		}
		//25.load the main game 
		if (GUI.Button (new Rect (Screen.width / 2 + 430f, Screen.height / 2 + 0f, 100f, 50f), "Quit")) {
			//26. load the main scene (which is 1 in the build settings)
			Application.Quit ();
